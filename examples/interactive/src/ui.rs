...
use crate::{
    ...
    HANDLE_OBSTACLE_MATERIAL, HANDLE_OBSTACLE_MESH,
};
...
fn button_system(
    ...
    for (interaction, button, relative_position) in &mut interaction_query {
        match (*interaction, interaction.is_changed(), button) {
            ...
            (Interaction::Pressed, true, UiButton::AddRandomObstacles) => {
                let mut thread_rng = rand::thread_rng();
                for _i in 0..10 {
                    let x = thread_rng.gen_range(-BOARD_LIMIT..BOARD_LIMIT);
                    let y = thread_rng.gen_range(-0.21..-0.19);
                    let z = thread_rng.gen_range(-BOARD_LIMIT..BOARD_LIMIT);
            ...
            (Interaction::Pressed, true, UiButton::AddObstacleGrid) => {
                let count = 10;
                let mut thread_rng = rand::thread_rng();
                        ...
                        commands.spawn((
                            PbrBundle {
                                ...
                                transform: Transform::from_xyz(
                                    ...
                                    y,
                                    (j as f32 / count as f32 - 0.5) * 2.0 * BOARD_LIMIT,
                                )
                                .with_rotation(Quat::from_rotation_y(rot)),
                                ..Default::default()
                            },
